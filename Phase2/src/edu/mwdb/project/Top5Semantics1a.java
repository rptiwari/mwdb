...
public class Top5Semantics1a {
	...
	public void getSemantics(String personId, String model) {
		...
		try 
		{
			...
			{
				finalKeywordsList.add(k.getKey());
			}
			...
			if(model.equalsIgnoreCase("PCA")){
				...
				for(int i=0;i<columnSize;i++)
				{
					for(int j=0;j<5;j++)
					{
						System.out.print(finalKeywordsList.get(i) + "\t" + resultSematicMatrixPCA[i][j] + "\t");
					}
					System.out.println();
				}
			}
			...
			if(model.equalsIgnoreCase("SVD")){
				...
				for(int i=0;i<5;i++)
				{
					...
					{
						System.out.print(finalKeywordsList.get(j) + "\t" + resultSematicMatrixSVD[i][j] + "\t");
					}
					System.out.println();
				}
			}
